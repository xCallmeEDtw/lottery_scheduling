...
#define MAX_TICKETS 20 /* CHANGED: maximum number of tickets that can be assigned */
#define PICK_ANY	1
...
/* MODIFIY: implement random function */
int ProcessSetPriority(int n) {

	  return (unsigned int)(random() %n )+1; /* CHANGED: variable that receives the drawn ticket */
}
...
void proc_init(void)
{
	...
	for (rp = BEG_PROC_ADDR, i = -NR_TASKS; rp < END_PROC_ADDR; ++rp, ++i) {
		...
		rp->p_tickets = 5 ;		/* MODIFY: variable to store process tickets */	
        rp->p_pritickets = 5 ;		/* MODIFY: variable to store process tickets priority */
        rp->p_runtimes = 0 ;
 ...
 */
  ...
  register struct proc *aux; /* CHANGED: pointer that will move through the queues */
  int total_tickets = 0; /* CHANGED: variable to store the total number of tickets */
  rdy_head = get_cpulocal_var(run_q_head);

  /* CHANGED: go through all processes, assigning tickets and adding the variable that accumulates the total */
  for (q=0; q <= 14; q++) {
    aux = rdy_head[q];
    while(aux) {  
		/*Traversing the priority queue a Removing from MAX_TICKETS, the current priority value; this way, */
		/*the higher the priority (lower value), the greater the number of tickets*/
        
       /*aux->p_tickets = MAX_TICKETS - q; */
       if (aux->p_runtimes!=1) aux->p_runtimes =0;
	   if ( aux->p_pritickets < 1  || aux->p_pritickets > maxtickets ){
	       aux->p_tickets = 5;
		   aux->p_pritickets = aux->p_tickets;
	   }
	   else  {
		  aux->p_tickets = aux->p_pritickets;
	   }
  ...
  if(total_tickets > 0) { /* if you have a user process ready */
    /* CHANGED: holding the lottery */

	int tickets_num = ProcessSetPriority(total_tickets);
    int winner = 0; /* CHANGED: variable to accumulate tickets, until the winner is found */
    ...
    for (q=0; q <= 14; q++) {
        ...
        while(aux) {
            ...
            if(winner >= tickets_num && setflagticket == 1  ) {

				if (aux->p_pritickets > 1)
				   aux->p_pritickets=aux->p_pritickets;
				   setflagticket = 2;
            }
			else {
				if (aux->p_pritickets < maxtickets  && !proc_no_quantum(aux) && aux->p_runtimes>0 ){

				   aux->p_pritickets=aux->p_pritickets+1;
				   aux->p_runtimes=0;
				}
				if (aux->p_pritickets > mintickets  && proc_no_quantum(aux) && aux->p_runtimes>0 ){
				   aux->p_pritickets=aux->p_pritickets-1;
				   aux->p_runtimes=0;
				}
			}
